                    ...
                    if "routes" in dados_plano:
                        ...
                        if plano is None:
                            ...
                            for i in range(len(coords) - 1):
                                ...
                                rumo = (math.degrees(math.atan2(dlng, dlat)) + 360) % 360
                                dist = math.sqrt(dlng**2 + dlat**2) * dist_factor  # sempre em km

                                dt += dist

                                # Velocidade: converte mph→km/h se imperial
                                vel_raw = int(speeds[i]) if i < len(speeds) else int(plano.get("speed", 450))
                                vel_p   = round(vel_raw * 1.60934) if is_imperial else vel_raw

                                # Altitude: converte ft→m se imperial
                                alt_raw = int(altitudes[i+1]) if i+1 < len(altitudes) else int(plano.get("altitude", 3000))
                            ...
                            st.success(f"✅ {len(nl)} pernas de '{plano.get('name','Rota')}' [{unidade_txt}] → NavLog atualizado!")
                except Exception as e:
                    st.error(f"Erro: {e}")
    with col_clear:
